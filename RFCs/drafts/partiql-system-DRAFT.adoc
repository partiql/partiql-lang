...
. **identifier**: A reference to something such as a _catalog_ or _object_.
. **CRUD**: The fundamental data operations e.g. INSERT / SELECT / UPDATE / DELETE 

[loweralpha,title="Catalogs",start=4]
...
. **name**: An identifier associated with a _catalog object_.

[loweralpha,title="Session",start=11]
...
. **path**: A sequence of _scopes_ used for name resolution.
...
A _PartiQL-client_ interacts with a _PartiQL-system_ by invoking statements with a given _session_. This session contains information about the client's state such as the current user, current scope, and session path. The current user can be used for permissions, but authorization in a _PartiQL-system_ is, at present, not defined.

The current scope and session path determine how names (tables, functions, etc.) are resolved in a statement. Names may be _absolute_ or _relative_ ... 
