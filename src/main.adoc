= PartiQL Specification
The PartiQL Specification Committee
:doctype: book
:creator: {author}
:revnumber: v0.3.0-HEAD
:version-label!:
:revremark: PRE-RELEASE. See official releases at https://github.com/partiql/partiql-lang/releases
:copyright: Copyright ©2019 Amazon.com Inc. or Affiliates (“Amazon”)
:docinfo:
:sectnums:
:sectanchors:
:sectlinks:
:sectnumlevels: 5
:toc: left
:toclevels: 2
:toc-title: Contents
:pagenums:
:imagesdir: ../build/images
:title-page:
:title-logo-image: image:PartiQL_Logo.svg[align=center,pdfwidth=50vw]
:title-page-background-image: image:bg-draft.svg[]
:icons: font
:source-highlighter: pygments
:pygments-style: colorful
:listing-caption: Listing
:example-caption: Example
:pdfmark: true
:xrefstyle: short
:stem: asciimath

// TODO html stylesheet; see https://docs.asciidoctor.org/asciidoctor/latest/html-backend/custom-stylesheet/
// TODO contribute syntax highlighter for partiql/ion to rogue


include::license.adoc[]
include::preamble.adoc[]
include::introduction.adoc[]
include::gradual_typing.adoc[]
include::execution_modes.adoc[]
include::types.adoc[]
include::type_conversions.adoc[]
include::database_environments.adoc[]
include::name_resolution.adoc[]
include::functions.adoc[]
include::query_syntax.adoc[]
include::subqueries.adoc[]
include::appendix.adoc[]
